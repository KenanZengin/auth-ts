...
export const generateVerificationToken = async(
    ...
    const token = uuidv4();
    const expires = new Date(new Date( new Date().getTime() + 3500 *1000)); // 1 saat süresi olcak tokenın
...
export const generateTwoFactorToken = async (email: string) => {
    ...
    const expires = new Date(new Date().getTime() + 5 * 60 * 1000); // => 5dk süresi var artık
